...
# Function to check if sdc disk is present
check_sdc() {
  lsblk | grep -q "^sdc"
}

# Function to check if /tmp is in /etc/fstab
check_fstab_tmp() {
  grep -q " /tmp " /etc/fstab
}

# Check if sdc disk is present
if check_sdc; then
  echo "sdc disk is present."
else
  echo "sdc disk is not present. Exiting."
  exit 1
fi

# Check if /tmp is already a separate partition in fstab
if check_fstab_tmp; then
  echo "/tmp is already a separate partition in /etc/fstab. Exiting."
  exit 1
else
  # Prepare the sdc disk for /tmp partition
  echo "Creating a filesystem on /dev/sdc..."
  mkfs.ext4 /dev/sdc

  # Backup fstab
  cp /etc/fstab /etc/fstab.bak

  # Mount the new /tmp partition with nodev, noexec, and nosuid options
  echo "Updating /etc/fstab..."
  echo "/dev/sdc /tmp ext4 defaults,nodev,noexec,nosuid 0 0" >> /etc/fstab

  # Create /tmp directory if it doesn't exist
  mkdir -p /tmp

  # Mount the /tmp partition
  echo "Mounting the new /tmp partition..."
  mount /tmp

  # Verify the mount options
  echo "Verifying the /tmp partition..."
  mount | grep /tmp

  echo "/tmp partition is now a separate partition with nodev, noexec, and nosuid options set."
fi
